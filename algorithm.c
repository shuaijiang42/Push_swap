...
/*   Updated: 2023/06/23 19:17:59 by shujiang         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	push_target_b(t_node **stack_a, t_node **stack_b, t_range *range)
{
	...
	free(target);
	if ((*stack_b)->pos < range->half && (*stack_b)->next
		&& (*stack_b)->next->pos >= range->half)
	{
		target = get_target(stack_a, range);
		if (target &&!(target->dir) && (*stack_a)
			&& (*stack_a)->pos >= range->max)
